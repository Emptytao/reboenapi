...
export type VideoTemplateId = 'grok' | 'omni'
...
export type VideoEndpointFamily = 'video-generations' | 'videos'
export type VideoInputAssetKind = 'image' | 'video' | 'audio'
export type VideoInputTransport = 'multipart-file' | 'base64-json' | 'url' | 'metadata'
export type VideoResolutionEncoding = 'width-height' | 'token'

export interface VideoInputCapability {
  min: number
  max: number
  transport: VideoInputTransport
  fieldNames: string[]
}

export interface VideoModelCapability {
  endpointFamily: VideoEndpointFamily
  imageInput: VideoInputCapability
  videoInput: VideoInputCapability
  audioInput: VideoInputCapability
  durationOptions: number[]
  resolutionOptions: string[]
  resolutionEncoding: VideoResolutionEncoding
...
export interface ApiProfileVideoConfig {
  templateId?: VideoTemplateId
  availableModels: string[]
  capabilityOverrides: Record<string, VideoTemplateConfig>
}
...
export interface ApiProfile {
  ...
  videoConfig?: ApiProfileVideoConfig
...
export interface ApiProfileBindings {
  gallery: string
  agent: string
  video: string
}
...
export interface AppSettings {
  ...
  profileBindings?: ApiProfileBindings
}
...
export interface VideoDraft {
  ...
  resolution: string
  imageAssets: VideoInputAsset[]
  videoAssets: VideoInputAsset[]
  audioAssets: VideoInputAsset[]
  updatedAt: number
}
...
export interface VideoTaskRecord {
  ...
  inputAudioAssetIds: string[]
  remoteTaskId?: string
  remoteStatus?: string | null
  resultUrl?: string | null
  rawSubmitPayload?: string
  rawStatusPayload?: string
  rawResponsePayload?: string
