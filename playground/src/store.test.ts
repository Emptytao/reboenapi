...
import type { AgentConversation, ExportData, StoredImage, StoredImageThumbnail, StoredMediaFile, TaskRecord, VideoTaskRecord } from './types'
...
vi.mock('./lib/db', () => {
  ...
  const mediaFiles = new Map<string, StoredMediaFile>()
  let imageSeq = 0
  ...
  return {
    ...
    getAllVideoTasks: async () => [...videoTasks.values()],
    putVideoTask: async (task: VideoTaskRecord) => {
    ...
    getMediaFile: async (id: string) => mediaFiles.get(id),
    getAllMediaFiles: async () => [...mediaFiles.values()],
    getAllMediaFileIds: async () => [...mediaFiles.keys()],
    putMediaFile: async (file: StoredMediaFile) => {
      mediaFiles.set(file.id, file)
      return file.id
    },
    deleteMediaFile: async (id: string) => {
      mediaFiles.delete(id)
    },
    clearMediaFiles: async () => {
      mediaFiles.clear()
    },
    ...
    storeMediaFile: async (file: File, kind: StoredMediaFile['kind']) => {
      const id = `stored-media-${++imageSeq}`
      mediaFiles.set(id, {
        id,
        kind,
        name: file.name,
