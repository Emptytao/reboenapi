...
import { DEFAULT_SETTINGS, getActiveApiProfile, getApiProfileForMode, getCustomProviderDefinition, isOpenAICompatibleProvider, mergeImportedSettings, normalizeProfileBindingsForSettings, normalizeSettings, validateApiProfile } from './lib/apiProfiles'
...
import {
  ...
  putVideoTask as dbPutVideoTask,
  deleteTask as dbDeleteTask,
  deleteVideoTask as dbDeleteVideoTask,
  clearTasks as dbClearTasks,
  clearVideoTasks as dbClearVideoTasks,
...
import { getVideoTaskSnapshot, refreshVideoModels, submitVideoTask } from './lib/videoApi'
import { getDefaultVideoTemplateId, getVideoModelCapability, getVideoTemplate, getVideoTemplateForModel, normalizeApiProfileVideoConfig, normalizeVideoTemplateConfig } from './lib/videoCapabilities'
...
const VIDEO_RECOVERY_POLL_MS = 10_000
const VIDEO_TASK_TIMEOUT_MS = 900_000
...
function getCachedMediaObjectUrl(id: string) {
  return mediaObjectUrlCache.get(id)
}

function getOrCreateMediaObjectUrl(file: StoredMediaFile) {
  const cached = mediaObjectUrlCache.get(file.id)
  if (cached) return cached
  const objectUrl = URL.createObjectURL(file.blob)
  mediaObjectUrlCache.set(file.id, objectUrl)
  return objectUrl
}

function releaseMediaObjectUrl(id: string) {
  const objectUrl = mediaObjectUrlCache.get(id)
  if (objectUrl) {
    try {
      URL.revokeObjectURL(objectUrl)
    } catch {
      // object URL 释放失败不影响主流程。
    }
  }
  mediaObjectUrlCache.delete(id)
}

function buildVideoInputAsset(file: StoredMediaFile): VideoInputAsset {
  return {
    id: file.id,
    mediaFileId: file.id,
    kind: file.kind,
    ...
    size: file.size,
    objectUrl: getOrCreateMediaObjectUrl(file),
  }
}
...
export function migratePersistedState(persistedState: unknown): unknown {
  ...
  return {
    ...
    videoDraft: normalizeVideoDraft(persistedState.videoDraft, typeof persistedState?.settings === 'object' && persistedState.settings && 'model' in persistedState.settings
      ? String((persistedState.settings as Record<string, unknown>).model ?? '')
      : ''),
  }
}
...
export function getPersistedState(state: AppState) {
  ...
  const videoDraft = getPersistableVideoDraft(state.videoDraft)
  return {
    settings,
    params: state.params,
    ...
    galleryInputDraft: settings.persistInputOnRestart && galleryInputDraft
      ...
      : null,
    videoDraft: settings.persistInputOnRestart ? videoDraft : createEmptyVideoDraft(state.videoDraft.modelId),
...
function mergePersistedState(persistedState: unknown, currentState: AppState): AppState {
  ...
  const galleryInputDraft = settings.persistInputOnRestart
    ...
    : null
  const videoDraft = settings.persistInputOnRestart
    ? normalizeVideoDraft(persisted.videoDraft, settings.model)
    : createEmptyVideoDraft(settings.model)
  ...
  const syncedSettings = (() => {
    const profileBindings = normalizeProfileBindingsForSettings(settings)
    const boundProfileId = profileBindings[appMode]
    return boundProfileId === settings.activeProfileId
      ? settings
      : normalizeSettings({ ...settings, activeProfileId: boundProfileId })
  })()
  return {
    ...currentState,
    ...persisted,
...
interface AppState {
  ...
  setVideoDraftAssets: (kind: VideoInputAssetKind, assets: VideoInputAsset[]) => void
  removeVideoDraftImage: (index: number) => void
  clearVideoDraftImages: () => void
  moveVideoDraftImage: (fromIdx: number, toIdx: number) => void
  ...
  setVideoTasks: (tasks: VideoTaskRecord[]) => void
...
function createEmptyVideoDraft(modelId = ''): VideoDraft {
  return {
    modelId,
    imageUrl: '',
    prompt: '',
...
function copyVideoInputAsset(asset: VideoInputAsset): VideoInputAsset {
  return { ...asset }
}

function copyVideoDraft(draft: VideoDraft): VideoDraft {
  return {
    ...
    imageAssets: draft.imageAssets.map(copyVideoInputAsset),
    videoAssets: draft.videoAssets.map(copyVideoInputAsset),
    audioAssets: draft.audioAssets.map(copyVideoInputAsset),
    updatedAt: draft.updatedAt,
  }
}

function normalizeVideoInputAsset(value: unknown, fallbackKind: VideoInputAssetKind): VideoInputAsset | null {
  if (!isRecord(value)) return null
  if (typeof value.id !== 'string' || !value.id) return null
  const mediaFileId = typeof value.mediaFileId === 'string' && value.mediaFileId ? value.mediaFileId : value.id
  const kind = value.kind === 'image' || value.kind === 'video' || value.kind === 'audio' ? value.kind : fallbackKind
  const name = typeof value.name === 'string' && value.name.trim() ? value.name.trim() : mediaFileId
  ...
  const objectUrl = typeof value.objectUrl === 'string' ? value.objectUrl : ''
  const dataUrl = typeof value.dataUrl === 'string' ? value.dataUrl : undefined
  return {
    id: typeof value.id === 'string' ? value.id : mediaFileId,
    mediaFileId,
    kind,
    name,
    mimeType,
...
function normalizeVideoDraft(value: unknown, fallbackModelId = ''): VideoDraft {
  if (!isRecord(value)) return createEmptyVideoDraft(fallbackModelId)
  const durationSeconds = typeof value.durationSeconds === 'number' && Number.isFinite(value.durationSeconds)
    ? Math.max(0, Math.trunc(value.durationSeconds))
    : null
  return {
    modelId: typeof value.modelId === 'string' ? value.modelId : fallbackModelId,
    imageUrl: typeof value.imageUrl === 'string' ? value.imageUrl : '',
    prompt: typeof value.prompt === 'string' ? value.prompt : '',
    durationSeconds,
    resolution: typeof value.resolution === 'string' ? value.resolution : '',
    imageAssets: Array.isArray(value.imageAssets)
      ? value.imageAssets.map((asset) => normalizeVideoInputAsset(asset, 'image')).filter((asset): asset is VideoInputAsset => Boolean(asset))
      : [],
    videoAssets: Array.isArray(value.videoAssets)
      ? value.videoAssets.map((asset) => normalizeVideoInputAsset(asset, 'video')).filter((asset): asset is VideoInputAsset => Boolean(asset))
      : [],
    audioAssets: Array.isArray(value.audioAssets)
      ? value.audioAssets.map((asset) => normalizeVideoInputAsset(asset, 'audio')).filter((asset): asset is VideoInputAsset => Boolean(asset))
      : [],
    updatedAt: typeof value.updatedAt === 'number' ? value.updatedAt : Date.now(),
  }
}

function getPersistableVideoDraft(draft: VideoDraft) {
  return {
    ...copyVideoDraft(draft),
    imageAssets: draft.imageAssets.map((asset) => ({ ...asset, objectUrl: '' })),
    videoAssets: draft.videoAssets.map((asset) => ({ ...asset, objectUrl: '' })),
    audioAssets: draft.audioAssets.map((asset) => ({ ...asset, objectUrl: '' })),
  }
}

function normalizePersistedVideoTaskStatus(value: unknown, resultUrl: unknown, error: unknown): VideoTaskRecord['status'] {
  if (value === 'queued' || value === 'running' || value === 'done' || value === 'error') return value
  if (typeof error === 'string' && error.trim()) return 'error'
  if (typeof resultUrl === 'string' && resultUrl.trim()) return 'done'
  if (typeof value === 'string') {
    const normalized = value.trim().toLowerCase()
    if (['success', 'succeeded', 'completed', 'done', 'finished', 'ok'].includes(normalized)) return 'done'
    if (['failed', 'failure', 'error', 'cancelled', 'canceled', 'rejected', 'expired'].includes(normalized)) return 'error'
    if (normalized === 'queued') return 'queued'
  }
  return 'running'
}

function normalizeVideoTaskRecord(value: unknown): VideoTaskRecord | null {
  if (!isRecord(value) || typeof value.id !== 'string' || !value.id.trim()) return null
  if (typeof value.prompt !== 'string') return null
  const params = isRecord(value.params) ? value.params : {}
  const status = normalizePersistedVideoTaskStatus(value.status ?? value.remoteStatus, value.resultUrl, value.error)

  return {
    id: value.id,
    prompt: value.prompt,
    params: {
      durationSeconds: typeof params.durationSeconds === 'number' && Number.isFinite(params.durationSeconds)
        ? Math.max(0, Math.trunc(params.durationSeconds))
        : null,
    ...
    apiProfileId: typeof value.apiProfileId === 'string' ? value.apiProfileId : undefined,
    apiProfileName: typeof value.apiProfileName === 'string' ? value.apiProfileName : undefined,
    apiModel: typeof value.apiModel === 'string' ? value.apiModel : '',
    templateId: getVideoTemplate(typeof value.templateId === 'string' ? value.templateId : null)?.id ?? getDefaultVideoTemplateId(),
    endpointFamily: value.endpointFamily === 'videos' ? 'videos' : 'video-generations',
    inputImageAssetIds: normalizeStringArray(isRecord(value) ? value.inputImageAssetIds ?? value.inputImageIds : undefined),
    inputVideoAssetIds: normalizeStringArray(isRecord(value) ? value.inputVideoAssetIds ?? value.inputVideoIds : undefined),
    inputAudioAssetIds: normalizeStringArray(isRecord(value) ? value.inputAudioAssetIds ?? value.inputAudioIds : undefined),
    ...(typeof value.remoteTaskId === 'string' && value.remoteTaskId.trim() ? { remoteTaskId: value.remoteTaskId } : {}),
    remoteStatus: typeof value.remoteStatus === 'string' ? value.remoteStatus : null,
    resultUrl: typeof value.resultUrl === 'string' ? value.resultUrl : null,
    ...(typeof value.rawSubmitPayload === 'string' ? { rawSubmitPayload: value.rawSubmitPayload } : {}),
    ...(typeof value.rawStatusPayload === 'string' ? { rawStatusPayload: value.rawStatusPayload } : {}),
    ...(typeof value.rawResponsePayload === 'string' ? { rawResponsePayload: value.rawResponsePayload } : {}),
    status,
...
function normalizeVideoTasks(value: unknown): VideoTaskRecord[] {
  if (!Array.isArray(value)) return []
  return value.map(normalizeVideoTaskRecord).filter((task): task is VideoTaskRecord => Boolean(task))
}

function getVideoTaskMediaIds(task: VideoTaskRecord) {
  return uniqueIds([
    ...task.inputImageAssetIds,
    ...task.inputVideoAssetIds,
    ...task.inputAudioAssetIds,
  ])
}

function addVideoTaskReferencedMediaIds(target: Set<string>, task: VideoTaskRecord) {
  for (const id of getVideoTaskMediaIds(task)) target.add(id)
}

function addVideoDraftReferencedMediaIds(target: Set<string>, draft: VideoDraft | null) {
  if (!draft) return
  for (const asset of [...draft.imageAssets, ...draft.videoAssets, ...draft.audioAssets]) {
    if (asset.mediaFileId) target.add(asset.mediaFileId)
  }
}

function hydrateVideoDraftAssets(draft: VideoDraft, mediaFiles: Map<string, StoredMediaFile>) {
  let changed = false
  const restore = (assets: VideoInputAsset[]) => assets
    .map((asset) => {
      const file = mediaFiles.get(asset.mediaFileId)
      if (!file) {
        changed = true
        return null
      }
      const objectUrl = getOrCreateMediaObjectUrl(file)
      if (asset.objectUrl !== objectUrl) changed = true
      return { ...asset, objectUrl, name: file.name, mimeType: file.mimeType, size: file.size, kind: file.kind }
    })
    .filter((asset): asset is VideoInputAsset => Boolean(asset))

  const nextDraft: VideoDraft = {
    ...draft,
    imageAssets: restore(draft.imageAssets),
    videoAssets: restore(draft.videoAssets),
    audioAssets: restore(draft.audioAssets),
  }
  return { draft: nextDraft, changed }
}

async function deleteUnreferencedMediaFileIds(mediaFileIds: Iterable<string>) {
  const state = useStore.getState()
  const stillUsed = new Set<string>()
  for (const task of state.videoTasks) addVideoTaskReferencedMediaIds(stillUsed, task)
  addVideoDraftReferencedMediaIds(stillUsed, state.videoDraft)

  const uniqueIdsToDelete = uniqueIds(Array.from(mediaFileIds))
  ...
  persist(
    (set, get) => ({
      ...
      setAppMode: (appMode) => {
        ...
        const settings = normalizeSettings(state.settings)
        const activeProfile = getApiProfileForMode(settings, appMode)
      ...
      setSettings: (s) => set((st) => {
        ...
        const settings = normalizeSettings({
          ...merged,
          profileBindings: merged.profileBindings ?? previous.profileBindings,
        })
      ...
      videoDraft: createEmptyVideoDraft(),
      setVideoDraft: (patch) => set((state) => ({
        videoDraft: {
          ...state.videoDraft,
          ...patch,
          updatedAt: Date.now(),
      ...
      setVideoDraftAssets: (kind, assets) => set((state) => ({
        videoDraft: {
          ...state.videoDraft,
          updatedAt: Date.now(),
          ...(kind === 'image'
            ? { imageAssets: assets.map(copyVideoInputAsset) }
            : kind === 'video'
            ? { videoAssets: assets.map(copyVideoInputAsset) }
            : { audioAssets: assets.map(copyVideoInputAsset) }),
        },
      })),
      removeVideoDraftImage: (index) => set((state) => {
        const removed = state.videoDraft.imageAssets[index]
        if (!removed) return state
        const imageAssets = state.videoDraft.imageAssets.filter((_, itemIndex) => itemIndex !== index)
        const previousImages = state.videoDraft.imageAssets.map((asset) => ({ id: asset.id, dataUrl: asset.objectUrl }))
        const nextImages = imageAssets.map((asset) => ({ id: asset.id, dataUrl: asset.objectUrl }))
        void deleteUnreferencedMediaFileIds([removed.mediaFileId])
        return {
          videoDraft: {
            ...state.videoDraft,
            imageAssets: imageAssets.map(copyVideoInputAsset),
            prompt: remapImageMentionsForOrder(state.videoDraft.prompt, previousImages, nextImages),
            updatedAt: Date.now(),
          },
        }
      }),
      clearVideoDraftImages: () => set((state) => {
        const removedIds = state.videoDraft.imageAssets.map((asset) => asset.mediaFileId)
        if (removedIds.length > 0) void deleteUnreferencedMediaFileIds(removedIds)
        return {
          videoDraft: {
            ...state.videoDraft,
            imageAssets: [],
            prompt: remapImageMentionsForOrder(
              state.videoDraft.prompt,
              state.videoDraft.imageAssets.map((asset) => ({ id: asset.id, dataUrl: asset.objectUrl })),
              [],
            ),
            updatedAt: Date.now(),
          },
      ...
      moveVideoDraftImage: (fromIdx, toIdx) => set((state) => {
        const images = [...state.videoDraft.imageAssets]
        if (fromIdx < 0 || fromIdx >= images.length) return state
        const normalizedToIdx = Math.max(0, Math.min(images.length, toIdx))
        if (normalizedToIdx === fromIdx || normalizedToIdx === fromIdx + 1) return state
        const previousImages = images.map((asset) => ({ id: asset.id, dataUrl: asset.objectUrl }))
        const [removed] = images.splice(fromIdx, 1)
        images.splice(fromIdx < normalizedToIdx ? normalizedToIdx - 1 : normalizedToIdx, 0, removed)
        const nextImages = images.map((asset) => ({ id: asset.id, dataUrl: asset.objectUrl }))
        return {
          videoDraft: {
            ...state.videoDraft,
            imageAssets: images.map(copyVideoInputAsset),
            prompt: remapImageMentionsForOrder(state.videoDraft.prompt, previousImages, nextImages),
            updatedAt: Date.now(),
          },
        }
      ...
      videoTasks: [],
      setVideoTasks: (videoTasks) => set({ videoTasks }),
...
function putVideoTask(task: VideoTaskRecord): Promise<IDBValidKey> {
  return dbPutVideoTask(task)
}

export function getCodexCliPromptKey(settings: AppSettings, mode: 'gallery' | 'agent' | 'video' = 'gallery'): string {
  const profile = getApiProfileForMode(settings, mode)
...
export function showCodexCliPrompt(force = false, reason = '接口返回的提示词已被改写', mode: 'gallery' | 'agent' = 'gallery') {
  const state = useStore.getState()
  const settings = normalizeSettings(state.settings)
  const profile = getApiProfileForMode(settings, mode)
  const promptKey = getCodexCliPromptKey(settings, mode)
  if (!force && (profile.codexCli || state.dismissedCodexCliPrompts.includes(promptKey))) return
    ...
    action: () => {
      ...
      state.setSettings(normalizeSettings({
        ...
        profiles: settings.profiles.map((item) =>
          item.id === profile.id ? { ...item, codexCli: true } : item,
        ),
      }))
    },
...
function createSettingsForApiProfile(settings: AppSettings, profile: ApiProfile, mode: 'gallery' | 'agent' | 'video' = 'gallery'): AppSettings {
  const normalized = normalizeSettings(settings)
  return normalizeSettings({
    ...
    profileBindings: {
      ...normalizeProfileBindingsForSettings(normalized),
      [mode]: profile.id,
    },
  })
}
...
function getVideoTaskApiProfile(settings: AppSettings, task: VideoTaskRecord): ApiProfile | null {
  const normalized = normalizeSettings(settings)
  if (!task.apiProfileId) return null
  const byId = normalized.profiles.find((profile) => profile.id === task.apiProfileId)
  if (byId && (!task.apiProvider || byId.provider === task.apiProvider || isOpenAICompatibleProvider(normalized, byId.provider))) {
    return byId
  }
  return null
}
...
function updateVideoTaskInStore(taskId: string, patch: Partial<VideoTaskRecord>) {
  const { videoTasks, setVideoTasks } = useStore.getState()
  const updated = videoTasks.map((task) => task.id === taskId ? { ...task, ...patch } : task)
  setVideoTasks(updated)
  const task = updated.find((item) => item.id === taskId)
  if (task) void putVideoTask(task)
}
...
function scheduleVideoRecovery(taskId: string, delayMs = VIDEO_RECOVERY_POLL_MS) {
  ...
  const timer = setTimeout(() => {
    videoRecoveryTimers.delete(taskId)
    void recoverVideoTask(taskId)
  }, delayMs)
  videoRecoveryTimers.set(taskId, timer)
}

async function completeRecoveredVideoTask(task: VideoTaskRecord, snapshot: Awaited<ReturnType<typeof getVideoTaskSnapshot>>) {
  const latest = useStore.getState().videoTasks.find((item) => item.id === task.id)
  if (!latest || (latest.status === 'done' && latest.resultUrl)) return

  updateVideoTaskInStore(task.id, {
    remoteTaskId: snapshot.remoteTaskId ?? task.remoteTaskId,
    remoteStatus: snapshot.remoteStatus,
    resultUrl: snapshot.resultUrl,
    rawStatusPayload: snapshot.rawResponsePayload,
    rawResponsePayload: snapshot.rawResponsePayload,
    status: snapshot.status,
    error: snapshot.status === 'error' ? snapshot.error : null,
    finishedAt: snapshot.status === 'done' || snapshot.status === 'error' ? Date.now() : task.finishedAt ?? null,
    elapsed: snapshot.status === 'done' || snapshot.status === 'error' ? Date.now() - task.createdAt : task.elapsed,
  })
  useStore.getState().showToast(snapshot.status === 'done' ? '视频任务已恢复' : '视频任务状态已更新', snapshot.status === 'done' ? 'success' : 'info')
}

export async function refreshVideoTaskResult(taskId: string): Promise<void> {
  const state = useStore.getState()
  const task = state.videoTasks.find((item) => item.id === taskId)
  if (!task) {
    state.showToast('视频任务不存在', 'error')
    return
  }
  if (!task.remoteTaskId) {
    state.showToast('该任务没有远端任务 ID，无法重新获取结果', 'error')
    return
  }

  const profile = getVideoTaskApiProfile(state.settings, task)
  if (!profile) {
    state.showToast('该任务对应的 API 配置已不存在，无法重新获取结果', 'error')
    return
  }

  clearVideoRecoveryTimer(taskId)
  try {
    state.showToast('正在重新获取视频结果...', 'info')
    const snapshot = await getVideoTaskSnapshot(profile, task.templateId ?? getDefaultVideoTemplateId(), task.remoteTaskId)
    await completeRecoveredVideoTask(task, snapshot)
    const latest = useStore.getState().videoTasks.find((item) => item.id === taskId)
    if (latest && (latest.status === 'queued' || latest.status === 'running') && latest.remoteTaskId) {
      scheduleVideoRecovery(taskId)
    }
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err)
    updateVideoTaskInStore(taskId, {
      error: message,
      rawStatusPayload: message,
    })
    state.showToast(`重新获取视频结果失败：${message}`, 'error')
  }
}

async function recoverVideoTask(taskId: string) {
  const { settings, videoTasks } = useStore.getState()
  const task = videoTasks.find((item) => item.id === taskId)
  if (!task || task.status === 'done' || !task.remoteTaskId) return

  if (Date.now() - task.createdAt >= VIDEO_TASK_TIMEOUT_MS) {
    clearVideoRecoveryTimer(taskId)
    updateVideoTaskInStore(taskId, {
      status: 'error',
      error: '视频生成超时（900 秒）',
  ...
  const profile = getVideoTaskApiProfile(settings, task)
  if (!profile) {
    scheduleVideoRecovery(taskId)
    return
  }
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err)
    if (/abort|network|failed to fetch|fetch failed|load failed|timeout|连接|断开|中断/i.test(message)) {
      scheduleVideoRecovery(taskId)
      return
    }
...
export async function initStore() {
  ...
  const [storedTasks, storedVideoTasks, storedMediaFiles] = await Promise.all([
    getAllTasks(),
    getAllVideoTasks(),
    getAllMediaFiles(),
  ])
  ...
  const videoTaskMap = new Map(storedVideoTasks.map((task) => [task.id, normalizeVideoTaskRecord(task)] as const))
  const normalizedVideoTasks = Array.from(videoTaskMap.values()).filter((task): task is VideoTaskRecord => Boolean(task))
  const mediaFileMap = new Map(storedMediaFiles.map((file) => [file.id, file] as const))
  const videoReferencedIds = new Set<string>()
  for (const task of normalizedVideoTasks) addVideoTaskReferencedMediaIds(videoReferencedIds, task)
  addVideoDraftReferencedMediaIds(videoReferencedIds, useStore.getState().videoDraft)

  const restoredVideoDraft = hydrateVideoDraftAssets(useStore.getState().videoDraft, mediaFileMap)
  if (restoredVideoDraft.changed) {
    useStore.getState().setVideoDraft(restoredVideoDraft.draft)
  }

  const persistedVideoTasks = normalizedVideoTasks.map((task) => {
    const nextTask = {
      ...task,
      params: {
        ...task.params,
        metadata: task.params.metadata ? { ...task.params.metadata } : undefined,
      },
    }
    return nextTask
  })
  await Promise.all(persistedVideoTasks.map((task) => putVideoTask(task)))
  useStore.getState().setVideoTasks(persistedVideoTasks)

  for (const task of persistedVideoTasks) {
    if ((task.status === 'running' || task.status === 'queued') && task.remoteTaskId) {
      scheduleVideoRecovery(task.id, 0)
    }
  }

  const mediaReferencedIds = new Set<string>()
  for (const task of persistedVideoTasks) addVideoTaskReferencedMediaIds(mediaReferencedIds, task)
  addVideoDraftReferencedMediaIds(mediaReferencedIds, useStore.getState().videoDraft)
  const storedMediaIds = await getAllMediaFileIds()
  const restoredMediaFiles = new Map<string, StoredMediaFile>()
  for (const mediaFile of storedMediaFiles) {
    if (!mediaReferencedIds.has(mediaFile.id)) {
      await deleteMediaFile(mediaFile.id)
      releaseMediaObjectUrl(mediaFile.id)
      continue
    }
    restoredMediaFiles.set(mediaFile.id, mediaFile)
    getOrCreateMediaObjectUrl(mediaFile)
  }
  const latestVideoDraft = hydrateVideoDraftAssets(useStore.getState().videoDraft, restoredMediaFiles)
  if (latestVideoDraft.changed) {
    useStore.getState().setVideoDraft(latestVideoDraft.draft)
  }
  if (storedMediaIds.length > 0 && storedMediaIds.some((id) => !restoredMediaFiles.has(id))) {
    // 孤立媒体已清理，无需额外操作。
  }
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  ...
  let activeProfile = getApiProfileForMode(normalizedSettings, 'gallery')
...
export async function submitAgentMessage() {
  ...
  const normalizedSettings = normalizeSettings(settings)
  const activeProfile = getApiProfileForMode(normalizedSettings, 'agent')
...
async function executeTask(taskId: string) {
  ...
  const activeProfile = taskProfile ?? getApiProfileForMode(settings, task.sourceMode === 'agent' ? 'agent' : 'gallery')
  ...
  try {
    ...
    if (latestTask.apiProvider === 'fal' && latestFalRequestInfo && isFalConnectionRecoverableError(err)) {
      ...
      const activeProfile = getApiProfileForMode(settings, latestTask.sourceMode === 'agent' ? 'agent' : 'gallery')
...
export async function retryTask(task: TaskRecord) {
  const { settings } = useStore.getState()
  const activeProfile = getApiProfileForMode(settings, 'gallery')
...
export async function reuseConfig(task: TaskRecord) {
  ...
  const normalizedSettings = normalizeSettings(settings)
  const currentProfile = getApiProfileForMode(normalizedSettings, 'gallery')
...
export async function clearData(options: ClearOptions = { clearConfig: true, clearTasks: true }) {
  const { setTasks, setVideoTasks, clearInputImages, clearMaskDraft, setSettings, setParams, showToast } = useStore.getState()

  if (options.clearTasks) {
    for (const timer of videoRecoveryTimers.values()) clearTimeout(timer)
    videoRecoveryTimers.clear()
    for (const id of mediaObjectUrlCache.keys()) releaseMediaObjectUrl(id)
    await dbClearTasks()
    await dbClearVideoTasks()
    ...
    useStore.setState({
      agentConversations: [],
      activeAgentConversationId: null,
      videoDraft: createEmptyVideoDraft(useStore.getState().videoDraft.modelId),
      supportPromptOpen: false,
      supportPromptSkippedForImportedData: false,
    })
    clearInputImages()
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    ...
    const videoTasks = options.exportTasks ? await getAllVideoTasks() : []
    const images = options.exportTasks ? await getAllImages() : []
    const mediaFiles = options.exportTasks ? await getAllMediaFiles() : []
    ...
    const mediaFileEntries: NonNullable<ExportData['mediaFiles']> = {}
    ...
    if (options.exportTasks) {
      for (const mediaFile of mediaFiles) {
        const path = `media/${mediaFile.id}.bin`
        mediaFileEntries[mediaFile.id] = {
          path,
          name: mediaFile.name,
          kind: mediaFile.kind,
        ...
        zipFiles[path] = [new Uint8Array(await mediaFile.blob.arrayBuffer()), { mtime: new Date(mediaFile.createdAt) }]
      }
    }
    ...
    if (options.exportTasks) {
      ...
      manifest.videoTasks = videoTasks
      ...
      manifest.mediaFiles = mediaFileEntries
    }
...
export async function importData(file: File, options: ImportOptions = { importConfig: true, importTasks: true }): Promise<boolean> {
  try {
    ...
    if (options.importTasks && data.mediaFiles) {
      for (const [id, info] of Object.entries(data.mediaFiles)) {
        const bytes = unzipped[info.path]
        if (!bytes) continue
        ...
        await putMediaFile({
          ...
          size: info.size,
          blob: new Blob([arrayBuffer], { type: info.mimeType }),
          createdAt: info.createdAt,
        })
      }
    }

    if (options.importTasks && data.videoTasks) {
      for (const task of normalizeVideoTasks(data.videoTasks)) {
        await putVideoTask(task)
      }
      useStore.getState().setVideoTasks(await getAllVideoTasks())
    }
    ...
    if (options.importTasks && data.tasks) {
      ...
      if (data.videoTasks?.length) msg += `，${data.videoTasks.length} 个视频任务`
...
function getVideoDraftAssetsByKind(draft: VideoDraft, kind: VideoInputAssetKind) {
  return kind === 'image' ? draft.imageAssets : kind === 'video' ? draft.videoAssets : draft.audioAssets
}

function createVideoSettingsPatch(profile: ApiProfile, patch: Partial<ApiProfile>) {
  const normalized = normalizeSettings(useStore.getState().settings)
  return {
    profiles: normalized.profiles.map((item) => item.id === profile.id ? { ...profile, ...patch } : item),
    activeProfileId: normalized.activeProfileId,
  }
}

export async function refreshActiveProfileVideoModels(): Promise<string[]> {
  const state = useStore.getState()
  const settings = normalizeSettings(state.settings)
  const profile = getApiProfileForMode(settings, 'video')

  if (!isOpenAICompatibleProvider(settings, profile.provider)) {
    state.showToast('当前配置不是 OpenAI 兼容接口，无法刷新视频模型', 'error')
    return []
  }
  ...
  try {
    ...
    state.setSettings(createVideoSettingsPatch(profile, {
      videoConfig: {
        templateId: profile.videoConfig?.templateId ?? getDefaultVideoTemplateId(),
        availableModels: result.models,
        capabilityOverrides: profile.videoConfig?.capabilityOverrides ?? {},
      },
    }))
    if (!state.videoDraft.modelId && result.models.length > 0) {
      state.setVideoDraft({ modelId: result.models.includes(profile.model) ? profile.model : result.models[0] })
    }
    state.showToast(`已刷新 ${result.models.length} 个视频模型`, 'success')
    return result.models
  } catch (err) {
    state.showToast(`刷新视频模型失败：${err instanceof Error ? err.message : String(err)}`, 'error')
    return []
  }
}

export async function addVideoAssetFromFile(file: File, kind: VideoInputAssetKind): Promise<VideoInputAsset | null> {
  const state = useStore.getState()
  const expectedPrefix = kind === 'image' ? 'image/' : kind === 'video' ? 'video/' : 'audio/'
  if (!file.type.startsWith(expectedPrefix)) {
    state.showToast(`请选择有效的${kind === 'image' ? '图片' : kind === 'video' ? '视频' : '音频'}文件`, 'error')
    return null
  }

  const stored = await storeMediaFile(file, kind)
  const mediaFile = await getMediaFile(stored.id)
  if (!mediaFile) {
    state.showToast('媒体文件保存失败', 'error')
    return null
  }

  const asset = buildVideoInputAsset(mediaFile)
  const currentAssets = getVideoDraftAssetsByKind(state.videoDraft, kind)
  if (currentAssets.some((item) => item.mediaFileId === asset.mediaFileId)) return asset
  state.setVideoDraftAssets(kind, [...currentAssets, asset])
  return asset
}

export async function removeVideoDraftAsset(kind: VideoInputAssetKind, assetId: string) {
  const state = useStore.getState()
  const assets = getVideoDraftAssetsByKind(state.videoDraft, kind)
  const removed = assets.find((asset) => asset.id === assetId || asset.mediaFileId === assetId)
  if (!removed) return
  state.setVideoDraftAssets(kind, assets.filter((asset) => asset.id !== removed.id && asset.mediaFileId !== removed.mediaFileId))
  await deleteUnreferencedMediaFileIds([removed.mediaFileId])
}

export async function removeVideoTaskRecord(task: VideoTaskRecord) {
  const { videoTasks, setVideoTasks, showToast } = useStore.getState()
  clearVideoRecoveryTimer(task.id)
  setVideoTasks(videoTasks.filter((item) => item.id !== task.id))
  await dbDeleteVideoTask(task.id)
  await deleteUnreferencedMediaFileIds(getVideoTaskMediaIds(task))
  showToast('视频任务已删除', 'success')
}

export async function submitVideoGeneration(): Promise<void> {
  const state = useStore.getState()
  const settings = normalizeSettings(state.settings)
  const profile = getApiProfileForMode(settings, 'video')
  const draft = copyVideoDraft(state.videoDraft)

  if (!isOpenAICompatibleProvider(settings, profile.provider)) {
    state.showToast('当前配置不是 OpenAI 兼容接口，无法提交视频任务', 'error')
    return
  }
  ...
  if (!prompt) {
    state.showToast('请输入视频提示词', 'error')
    return
  }

  const modelId = draft.modelId.trim()
  if (!modelId) {
    state.showToast('请选择视频模型', 'error')
    return
  }

  const capability = getVideoModelCapability(profile, modelId)
  const template = getVideoTemplateForModel(profile, modelId)
  if (!capability) {
    state.showToast('当前模型尚未绑定视频模板，请先前往设置补充', 'error')
    state.setShowSettings(true, 'video')
    return
  }
  if (!template) {
    state.showToast('当前模型绑定的视频模板不可用，请先前往设置补充', 'error')
    state.setShowSettings(true, 'video')
    return
  }

  const loadFiles = async (assets: VideoInputAsset[]) => {
    const files: StoredMediaFile[] = []
    for (const asset of assets) {
      const mediaFile = await getMediaFile(asset.mediaFileId)
      if (!mediaFile) throw new Error(`素材「${asset.name}」已不存在，请重新上传`)
  ...
  const task: VideoTaskRecord = {
    ...
    params: {
      durationSeconds: draft.durationSeconds,
      resolution: draft.resolution,
      ...(Object.keys(capability.metadataDefaults).length > 0 ? { metadata: capability.metadataDefaults } : {}),
    },
    ...
    inputImageAssetIds: draft.imageAssets.map((asset) => asset.mediaFileId),
    inputVideoAssetIds: draft.videoAssets.map((asset) => asset.mediaFileId),
    inputAudioAssetIds: draft.audioAssets.map((asset) => asset.mediaFileId),
    status: 'queued',
    error: null,
    createdAt: Date.now(),
    finishedAt: null,
  ...
  state.setVideoTasks([task, ...state.videoTasks])
  await putVideoTask(task)

  try {
    const [imageFiles, videoFiles, audioFiles] = await Promise.all([
      loadFiles(draft.imageAssets),
      loadFiles(draft.videoAssets),
      loadFiles(draft.audioAssets),
    ])

    const snapshot = await submitVideoTask({
      profile,
      templateId: template.id,
      modelId,
      prompt: stripImageMentionMarkers(prompt),
      capability,
      durationSeconds: draft.durationSeconds,
      resolution: draft.resolution,
      imageFiles,
    ...
    updateVideoTaskInStore(taskId, {
      remoteTaskId: snapshot.remoteTaskId ?? undefined,
      remoteStatus: snapshot.remoteStatus,
      resultUrl: snapshot.resultUrl,
      rawSubmitPayload: snapshot.rawResponsePayload,
      rawResponsePayload: snapshot.rawResponsePayload,
      status: snapshot.status,
      error: snapshot.status === 'error' ? snapshot.error : null,
      finishedAt: snapshot.status === 'done' || snapshot.status === 'error' ? Date.now() : null,
      elapsed: snapshot.status === 'done' || snapshot.status === 'error' ? Date.now() - task.createdAt : null,
    })

    if ((snapshot.status === 'queued' || snapshot.status === 'running') && snapshot.remoteTaskId) {
      scheduleVideoRecovery(taskId)
    }

    if (settings.clearInputAfterSubmit) {
      state.setVideoDraft({
        ...createEmptyVideoDraft(modelId),
        modelId,
      })
    }

    state.showToast(snapshot.status === 'error' ? (snapshot.error || '视频任务提交失败') : '视频任务已提交', snapshot.status === 'error' ? 'error' : 'success')
  } catch (err) {
    updateVideoTaskInStore(taskId, {
      status: 'error',
      error: err instanceof Error ? err.message : String(err),
      rawResponsePayload: err instanceof Error ? err.message : String(err),
      finishedAt: Date.now(),
      elapsed: Date.now() - task.createdAt,
    })
    state.showToast(`视频任务提交失败：${err instanceof Error ? err.message : String(err)}`, 'error')
  }
}
