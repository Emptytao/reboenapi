...
import { getDefaultVideoTemplateId, normalizeApiProfileVideoConfig } from './videoCapabilities'
...
function normalizeProfileBindings(value: unknown, profiles: ApiProfile[], fallbackActiveProfileId: string): ApiProfileBindings {
  const fallback: ApiProfileBindings = {
    gallery: fallbackActiveProfileId,
    agent: fallbackActiveProfileId,
    video: fallbackActiveProfileId,
  }
  if (!isRecord(value)) return fallback
  const profileIds = new Set(profiles.map((profile) => profile.id))
  const pick = (key: keyof ApiProfileBindings) => {
    const candidate = value[key]
    return typeof candidate === 'string' && profileIds.has(candidate) ? candidate : fallback[key]
  }
  return {
...
export function createDefaultOpenAIProfile(overrides: Partial<ApiProfile> = {}): ApiProfile {
  ...
  const videoConfig = overrides.videoConfig
...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  ...
  return {
    ...
    videoConfig: normalizeApiProfileVideoConfig(record.videoConfig) ?? defaults.videoConfig,
    providerDrafts: normalizeProviderDrafts(record.providerDrafts, customProviderIds),
  }
}

function getVideoConfigDedupKey(videoConfig: ApiProfile['videoConfig']): string {
  if (!videoConfig) return ''
  return JSON.stringify({
    templateId: videoConfig.templateId ?? getDefaultVideoTemplateId(),
    availableModels: [...videoConfig.availableModels].sort((a, b) => a.localeCompare(b)),
    capabilityOverrides: Object.fromEntries(
      Object.entries(videoConfig.capabilityOverrides)
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([modelId, capability]) => [
          modelId,
          {
            templateId: capability.templateId ?? videoConfig.templateId ?? getDefaultVideoTemplateId(),
            durationOptions: [...capability.durationOptions].sort((a, b) => a - b),
            resolutionOptions: [...capability.resolutionOptions].sort((a, b) => a.localeCompare(b)),
            metadataDefaults: Object.fromEntries(Object.entries(capability.metadataDefaults).sort(([a], [b]) => a.localeCompare(b))),
          },
        ] as const),
    ),
  })
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  const legacyProfile = createDefaultOpenAIProfile({
    ...
    videoConfig: normalizeApiProfileVideoConfig(record.videoConfig),
  })
  ...
  const shouldApplyLegacyRootOverrides = isDefaultOpenAIProfile(active)
  const activeWithLegacyOverrides = shouldApplyLegacyRootOverrides
    ? {
        ...active,
        baseUrl: typeof record.baseUrl === 'string' ? record.baseUrl : active.baseUrl,
        apiKey: typeof record.apiKey === 'string' ? record.apiKey : active.apiKey,
        model: typeof record.model === 'string' && record.model.trim() ? record.model : active.model,
        timeout: typeof record.timeout === 'number' && Number.isFinite(record.timeout) ? record.timeout : active.timeout,
        apiMode: record.apiMode === 'images' || record.apiMode === 'responses' ? record.apiMode : active.apiMode,
        codexCli: typeof record.codexCli === 'boolean' ? record.codexCli : active.codexCli,
        apiProxy: typeof record.apiProxy === 'boolean' ? record.apiProxy : active.apiProxy,
        streamImages: typeof record.streamImages === 'boolean' ? record.streamImages : active.streamImages,
        streamPartialImages: normalizeStreamPartialImages(record.streamPartialImages, active.streamPartialImages),
      }
    : active
  const normalizedProfiles = shouldApplyLegacyRootOverrides
    ? profiles.map((profile) => profile.id === activeProfileId ? activeWithLegacyOverrides : profile)
    : profiles
  const normalizedActive = normalizedProfiles.find((p) => p.id === activeProfileId) ?? normalizedProfiles[0]
  const profileBindings = normalizeProfileBindings(record.profileBindings, normalizedProfiles, activeProfileId)

  return {
    baseUrl: normalizedActive.baseUrl,
    apiKey: normalizedActive.apiKey,
    model: normalizedActive.model,
    timeout: normalizedActive.timeout,
    apiMode: normalizedActive.apiMode,
    codexCli: normalizedActive.codexCli,
    apiProxy: normalizedActive.apiProxy,
    streamImages: normalizedActive.streamImages,
    streamPartialImages: normalizedActive.streamPartialImages,
    customProviders,
...
export function getApiProfileForMode(settings: Partial<AppSettings> | unknown, mode: 'gallery' | 'agent' | 'video'): ApiProfile {
  const normalized = normalizeSettings(settings)
  const profileId = normalized.profileBindings?.[mode] ?? normalized.activeProfileId
  return normalized.profiles.find((profile) => profile.id === profileId) ?? getActiveApiProfile(normalized)
}

export function normalizeProfileBindingsForSettings(settings: Partial<AppSettings> | unknown): ApiProfileBindings {
  const normalized = normalizeSettings(settings)
  return normalized.profileBindings ?? {
    gallery: normalized.activeProfileId,
    agent: normalized.activeProfileId,
    video: normalized.activeProfileId,
  }
}
...
function getApiProfileDedupKey(profile: ApiProfile): string {
  return JSON.stringify([
    ...
    profile.model.trim(),
    profile.apiMode,
    getVideoConfigDedupKey(profile.videoConfig),
  ])
}
...
export function mergeImportedSettings(currentSettings: Partial<AppSettings> | unknown, importedSettings: Partial<AppSettings> | unknown): AppSettings {
  ...
  return normalizeSettings({
    ...
    profileBindings: current.profileBindings,
  })
}
...
export const DEFAULT_SETTINGS: AppSettings = normalizeSettings({
  ...
  profileBindings: {
    gallery: DEFAULT_OPENAI_PROFILE_ID,
    agent: DEFAULT_OPENAI_PROFILE_ID,
    video: DEFAULT_OPENAI_PROFILE_ID,
  },
})
