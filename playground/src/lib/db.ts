import type { AgentConversation, TaskRecord, StoredImage, StoredImageThumbnail, StoredMediaFile, VideoTaskRecord, VideoInputAssetKind } from '../types'
...
const STORE_MEDIA_FILES = 'mediaFiles'
...
function openDB(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      if (!db.objectStoreNames.contains(STORE_VIDEO_TASKS)) {
        db.createObjectStore(STORE_VIDEO_TASKS, { keyPath: 'id' })
      }
...
export function getAllVideoTasks(): Promise<VideoTaskRecord[]> {
  return dbTransaction(STORE_VIDEO_TASKS, 'readonly', (s) => s.getAll())
}

export function putVideoTask(task: VideoTaskRecord): Promise<IDBValidKey> {
  return dbTransaction(STORE_VIDEO_TASKS, 'readwrite', (s) => s.put(task))
}

export function deleteVideoTask(id: string): Promise<undefined> {
  return dbTransaction(STORE_VIDEO_TASKS, 'readwrite', (s) => s.delete(id))
}

export function clearVideoTasks(): Promise<undefined> {
  return dbTransaction(STORE_VIDEO_TASKS, 'readwrite', (s) => s.clear())
}
...
export function getMediaFile(id: string): Promise<StoredMediaFile | undefined> {
  return dbTransaction(STORE_MEDIA_FILES, 'readonly', (s) => s.get(id))
}

export function getAllMediaFiles(): Promise<StoredMediaFile[]> {
  return dbTransaction(STORE_MEDIA_FILES, 'readonly', (s) => s.getAll())
}

export function getAllMediaFileIds(): Promise<string[]> {
  return dbTransaction(STORE_MEDIA_FILES, 'readonly', (s) => s.getAllKeys()).then((keys) => keys.map(String))
}

export function putMediaFile(file: StoredMediaFile): Promise<IDBValidKey> {
  return dbTransaction(STORE_MEDIA_FILES, 'readwrite', (s) => s.put(file))
}

export function deleteMediaFile(id: string): Promise<undefined> {
  return dbTransaction(STORE_MEDIA_FILES, 'readwrite', (s) => s.delete(id))
}

export function clearMediaFiles(): Promise<undefined> {
  return dbTransaction(STORE_MEDIA_FILES, 'readwrite', (s) => s.clear())
}
...
export async function storeMediaFile(file: File, kind: VideoInputAssetKind): Promise<StoreMediaFileResult> {
  const id = await hashBlob(file)
  const existing = await getMediaFile(id)
  if (!existing) {
    await putMediaFile({
      id,
      kind,
  ...
  return {
    ...
    mimeType: existing?.mimeType ?? file.type ?? 'application/octet-stream',
