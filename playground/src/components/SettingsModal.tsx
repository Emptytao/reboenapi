...
import {
  ...
  getApiProviderLabel,
  getActiveApiProfile,
  getApiProfileForMode,
  normalizeProfileBindingsForSettings,
...
import VideoCapabilityManager from './VideoCapabilityManager'
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${dirty ? 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300' : 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300'}`}>
              {dirty ? '有未保存改动' : '已保存'}
            </span>
          </div>
        ...
        <div className="flex shrink-0 gap-2">
          <button
            ...
            onClick={onSave}
            disabled={!dirty}
            className="rounded-xl bg-blue-500 px-4 py-2 text-xs font-medium text-white transition hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-blue-500"
          >
            保存
          </button>
          <button
            type="button"
            onClick={onReset}
            className="rounded-xl border border-gray-200/70 bg-white px-4 py-2 text-xs font-medium text-gray-700 transition hover:bg-gray-50 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-gray-200 dark:hover:bg-white/[0.07]"
          >
            {resetLabel}
          </button>
        </div>
...
function getApiSettingsSlice(settings: AppSettings) {
  return {
    activeProfileId: settings.activeProfileId,
    profileBindings: normalizeProfileBindingsForSettings(settings),
...
function getGeneralSettingsSlice(settings: AppSettings) {
  return {
    ...
    taskCompletionNotification: settings.taskCompletionNotification,
  }
}

function getAgentSettingsSlice(settings: AppSettings) {
  return {
    agentMaxToolRounds: settings.agentMaxToolRounds,
    agentWebSearch: settings.agentWebSearch,
    agentScrollToBottomAfterSubmit: settings.agentScrollToBottomAfterSubmit,
    agentMathFormattingPrompt: settings.agentMathFormattingPrompt,
  }
}

function getVideoSettingsSlice(settings: AppSettings) {
  return {
    activeProfileId: settings.activeProfileId,
    profiles: settings.profiles.map((profile) => ({
      id: profile.id,
      model: profile.model,
      videoConfig: profile.videoConfig,
    })),
  }
}
...
export default function SettingsModal() {
  ...
  const [timeoutInput, setTimeoutInput] = useState(String(getApiProfileForMode(settings, appMode === 'agent' ? 'agent' : appMode === 'video' ? 'video' : 'gallery').timeout))
  ...
  const profileBindings = normalizeProfileBindingsForSettings(draft)
  ...
  const profileBindingOptions = draft.profiles.map((profile) => ({
    label: `${profile.name} · ${getApiProviderLabel(draft, profile.provider)}`,
    value: profile.id,
  }))
  const currentModeBindingLabel = appMode === 'agent' ? '文本（Agent）' : appMode === 'video' ? '视频' : '图片（画廊）'
  ...
  const apiDirty = stableStringify(getApiSettingsSlice(draft)) !== stableStringify(getApiSettingsSlice(normalizedSettings)) || apiKeyDraft !== activeProfile.apiKey || timeoutInput !== String(activeProfile.timeout)
  const videoDirty = stableStringify(getVideoSettingsSlice(draft)) !== stableStringify(getVideoSettingsSlice(normalizedSettings))
  const generalDirty = stableStringify(getGeneralSettingsSlice(draft)) !== stableStringify(getGeneralSettingsSlice(normalizedSettings))
  const normalizedAgentMaxToolRoundsInput = agentMaxToolRoundsInput.trim() === ''
    ? DEFAULT_AGENT_MAX_TOOL_ROUNDS
    : normalizeAgentMaxToolRounds(agentMaxToolRoundsInput, draft.agentMaxToolRounds)
  const agentDraftForComparison = { ...draft, agentMaxToolRounds: normalizedAgentMaxToolRoundsInput }
  const agentDirty = stableStringify(getAgentSettingsSlice(agentDraftForComparison)) !== stableStringify(getAgentSettingsSlice(normalizedSettings))
  const hasUnsavedModuleChanges = apiDirty || videoDirty || generalDirty || agentDirty
  ...
  useEffect(() => {
    ...
    const normalizedSettings = normalizeSettings(settings)
    const currentMode = appMode === 'agent' ? 'agent' : appMode === 'video' ? 'video' : 'gallery'
    const currentModeProfile = getApiProfileForMode(normalizedSettings, currentMode)
    const displaySettings = normalizedSettings.reuseTaskApiProfileTemporarily && reusedTaskApiProfileId && normalizedSettings.profiles.some((profile) => profile.id === reusedTaskApiProfileId)
      ...
      : normalizeSettings({ ...normalizedSettings, activeProfileId: currentModeProfile.id })
    ...
    setDraft(nextDraft)
    setTimeoutInput(String((nextDraft.profiles.find((profile) => profile.id === nextDraft.activeProfileId) ?? getActiveApiProfile(nextDraft)).timeout))
    setAgentMaxToolRoundsInput(String(nextDraft.agentMaxToolRounds))
  }, [apiProxyAvailable, apiProxyLocked, showSettings, settings, reusedTaskApiProfileId, appMode])
  ...
  useEffect(() => {
    setApiKeyDraft(activeProfile.apiKey)
  }, [activeProfile.apiKey, activeProfile.id])
  ...
  const setZipDownloadRouteEnabled = (route: ZipDownloadRoute, enabled: boolean) => {
    ...
    setDraft({ ...draft, zipDownloadRoutes: nextRoutes })
  }
  ...
  const saveApiKey = () => {
    updateActiveProfile({ apiKey: apiKeyDraft })
    showToast('API Key 已进入草稿，请点击 API 配置顶部的保存', 'info')
  }

  const clearApiKey = () => {
    setApiKeyDraft('')
    updateActiveProfile({ apiKey: '' })
    showToast('API Key 已清空草稿，请点击 API 配置顶部的保存', 'info')
  }

  const getDraftPreparedForSave = () => {
    ...
    return {
      ...
      profiles: draft.profiles.map((profile) =>
        profile.id === activeProfile.id
          ? { ...profile, apiKey: apiKeyDraft, timeout: activeProviderIsOpenAICompatible ? normalizedTimeout : profile.timeout }
          : profile,
      ),
    }
  }

  const saveModule = (moduleName: string) => {
    const preparedDraft = getDraftPreparedForSave()
    setAgentMaxToolRoundsInput(String(preparedDraft.agentMaxToolRounds))
    const preparedActiveProfile = preparedDraft.profiles.find((profile) => profile.id === activeProfile.id) ?? activeProfile
    setTimeoutInput(String(preparedActiveProfile.timeout))
    setApiKeyDraft(preparedActiveProfile.apiKey)
    commitSettings(preparedDraft)
    showToast(`${moduleName}已保存`, 'success')
  }

  const restoreGeneralDefaults = () => {
    setDraft({
      ...draft,
      enterSubmit: DEFAULT_SETTINGS.enterSubmit,
      clearInputAfterSubmit: DEFAULT_SETTINGS.clearInputAfterSubmit,
      referenceImageEditAction: DEFAULT_SETTINGS.referenceImageEditAction,
      zipDownloadRoutes: [...DEFAULT_SETTINGS.zipDownloadRoutes],
      persistInputOnRestart: DEFAULT_SETTINGS.persistInputOnRestart,
      reuseTaskApiProfileTemporarily: DEFAULT_SETTINGS.reuseTaskApiProfileTemporarily,
      alwaysShowRetryButton: DEFAULT_SETTINGS.alwaysShowRetryButton,
      taskCompletionNotification: DEFAULT_SETTINGS.taskCompletionNotification,
    })
  }

  const restoreAgentDefaults = () => {
    setAgentMaxToolRoundsInput(String(DEFAULT_SETTINGS.agentMaxToolRounds))
    setDraft({
      ...draft,
      agentMaxToolRounds: DEFAULT_SETTINGS.agentMaxToolRounds,
      agentWebSearch: DEFAULT_SETTINGS.agentWebSearch,
      agentScrollToBottomAfterSubmit: DEFAULT_SETTINGS.agentScrollToBottomAfterSubmit,
      agentMathFormattingPrompt: DEFAULT_SETTINGS.agentMathFormattingPrompt,
    })
  }

  const restoreApiDefaults = () => {
    const defaultProfile = createDefaultOpenAIProfile({ id: activeProfile.id, name: activeProfile.name || '默认' })
    const nextProfile = {
      ...defaultProfile,
      name: activeProfile.name || defaultProfile.name,
      videoConfig: activeProfile.videoConfig,
    }
    setApiKeyDraft('')
    setTimeoutInput(String(nextProfile.timeout))
    updateActiveProfile(nextProfile)
  }

  const restoreVideoDefaults = () => {
    updateActiveProfile({ videoConfig: undefined })
  }
  ...
  const handleClose = () => {
    ...
    if (hasUnsavedModuleChanges) {
      setConfirmDialog({
        title: '有未保存的配置',
        message: '当前设置页存在未保存改动。你可以保存全部后关闭，或放弃这些草稿改动。',
        buttons: [
          {
            label: '保存全部',
            action: () => {
              commitSettings(getDraftPreparedForSave())
  ...
  const commitAgentMaxToolRounds = useCallback(() => {
    ...
    if (value !== draft.agentMaxToolRounds) setDraft({ ...draft, agentMaxToolRounds: value })
  ...
  const toggleTaskCompletionNotification = async () => {
    if (draft.taskCompletionNotification) {
      setDraft({ ...draft, taskCompletionNotification: false })
      return
    }

    const result = await requestBrowserNotificationPermission()
    if (result.ok) {
      setDraft({ ...draft, taskCompletionNotification: true })
      showToast('通知权限已开启，请保存习惯配置后生效', 'success')
    } else {
  ...
  const updateProfileBinding = (mode: 'gallery' | 'agent' | 'video', profileId: string) => {
    const nextDraft = normalizeSettings({
      ...draft,
      profileBindings: {
        ...profileBindings,
  ...
  const deleteProfile = (id: string) => {
    ...
    const nextDraft = normalizeSettings({
      ...
      profileBindings: {
        gallery: profileBindings.gallery === id ? fallbackProfileId : profileBindings.gallery,
        agent: profileBindings.agent === id ? fallbackProfileId : profileBindings.agent,
        video: profileBindings.video === id ? fallbackProfileId : profileBindings.video,
      },
    })
    commitSettings(nextDraft)
  }
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'general' && (
              <div className="space-y-4">
                <SettingsModuleHeader
                  title="习惯配置"
                  description="提交方式、输入框行为、任务按钮和通知会先进入草稿，点击保存后统一生效。"
                  dirty={generalDirty}
                  onSave={() => saveModule('习惯配置')}
                  onReset={restoreGeneralDefaults}
                />
                  ...
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <div className="w-32">
                      <Select
                        ...
                        onChange={(val) => setDraft({ ...draft, enterSubmit: val === 'enter' })}
                        options={[
                  ...
                  <div className="mb-1 flex items-center justify-between gap-3">
                    ...
                    <div className="w-32">
                      <Select
                        ...
                        onChange={(val) => setDraft({ ...draft, referenceImageEditAction: val as AppSettings['referenceImageEditAction'] })}
                        options={[
                          { label: '询问', value: 'ask' },
                ...
                <div className="block">
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setDraft({ ...draft, persistInputOnRestart: !draft.persistInputOnRestart })}
                ...
                <div className="block">
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setDraft({ ...draft, reuseTaskApiProfileTemporarily: !draft.reuseTaskApiProfileTemporarily })}
                ...
                <div className="block">
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setDraft({ ...draft, alwaysShowRetryButton: !draft.alwaysShowRetryButton })}
            ...
            {activeTab === 'agent' && (
              <div className="space-y-4">
                <SettingsModuleHeader
                  title="Agent 配置"
                  description="Agent 的工具轮次、联网搜索、滚动和公式提示会先进入草稿，点击保存后统一生效。"
                  dirty={agentDirty}
                  onSave={() => saveModule('Agent 配置')}
                  onReset={restoreAgentDefaults}
                />
                  ...
                  <div className="mb-1 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setDraft({ ...draft, agentMaxToolRounds, agentWebSearch: !draft.agentWebSearch })
                      }}
                ...
                <div className="block">
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setDraft({ ...draft, agentScrollToBottomAfterSubmit: !draft.agentScrollToBottomAfterSubmit })}
                ...
                <div className="block">
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setDraft({ ...draft, agentMathFormattingPrompt: !draft.agentMathFormattingPrompt })}
            ...
            {activeTab === 'api' && (
              <div className="space-y-4">
                <SettingsModuleHeader
                  title="API 配置"
                  description="当前 profile 的 URL、Key、模型、接口模式和请求选项会先进入草稿。新增、复制、删除 profile 仍作为资源操作立即生效。"
                  dirty={apiDirty}
                  onSave={() => saveModule('API 配置')}
                  onReset={restoreApiDefaults}
                />
                <div>
                ...
                <div className="space-y-3 rounded-2xl border border-gray-200/70 bg-gray-50/70 p-3 dark:border-white/[0.08] dark:bg-white/[0.03]">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-medium text-gray-700 dark:text-gray-200">模式 API 绑定</div>
                      <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">图片、文本（Agent）和视频会分别使用这里绑定的配置。当前模式：{currentModeBindingLabel}</div>
                    </div>
                  </div>
                <div className="grid gap-3 md:grid-cols-3">
                  <label className="block">
                    <span className="mb-1.5 block text-xs text-gray-500 dark:text-gray-400">图片（画廊）</span>
                    <Select
                      value={profileBindings.gallery}
                      onChange={(value) => updateProfileBinding('gallery', String(value))}
                      options={profileBindingOptions}
                      disabled={defaultConfigOnly}
                      className="w-full min-w-0 rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                    />
                  </label>
                  <label className="block">
                    <span className="mb-1.5 block text-xs text-gray-500 dark:text-gray-400">文本（Agent）</span>
                    <Select
                      value={profileBindings.agent}
                      onChange={(value) => updateProfileBinding('agent', String(value))}
                      options={profileBindingOptions}
                      disabled={defaultConfigOnly}
                      className="w-full min-w-0 rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                    />
                  ...
                  <label className="block">
                    <span className="mb-1.5 block text-xs text-gray-500 dark:text-gray-400">视频</span>
                    <Select
                      value={profileBindings.video}
                      onChange={(value) => updateProfileBinding('video', String(value))}
                      options={profileBindingOptions}
                      disabled={defaultConfigOnly}
                      className="w-full min-w-0 rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                    />
                ...
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (!apiProxyLocked) updateActiveProfile({ apiProxy: !activeProfile.apiProxy })
                      }}
              ...
              <div className="block">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 pr-24 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50"
                  />
                  <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                    ...
                    <button type="button" onClick={saveApiKey} className="rounded-lg bg-blue-500 px-2.5 py-1.5 text-xs font-medium text-white transition hover:bg-blue-600">
                      保存
                    </button>
                    <button type="button" onClick={clearApiKey} className="rounded-lg border border-gray-200/70 bg-white px-2.5 py-1.5 text-xs font-medium text-gray-600 transition hover:bg-gray-50 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-gray-300 dark:hover:bg-white/[0.07]">
                      删除
                    </button>
                  </div>
                </div>
                <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                  支持通过查询参数覆盖：<code className="bg-gray-100 dark:bg-white/[0.06] px-1 py-0.5 rounded">?apiKey=</code>。保存后才会写入当前配置，删除会清空当前 profile 的 key。
                </div>
              </div>
                ...
                <div className="block space-y-3">
                  <div>
                    <div className="mb-1.5 flex items-center justify-between gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => updateActiveProfile({ streamImages: !activeProfile.streamImages })}
                  ...
                  <label className={`block ${activeProfile.streamImages ? '' : 'opacity-60'}`}>
                    ...
                    <Select
                      ...
                      onChange={(value) => updateActiveProfile({ streamPartialImages: normalizeStreamPartialImages(value) })}
              ...
              {activeProviderIsOpenAICompatible && (
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => updateActiveProfile({ responseFormatB64Json: !activeProfile.responseFormatB64Json })}
              ...
              {activeProfile.provider === 'openai' && (
                <div className="block">
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => updateActiveProfile({ codexCli: !activeProfile.codexCli })}
            ...
            {activeTab === 'video' && (
              <div className="space-y-4">
                <SettingsModuleHeader
                  title="视频配置"
                  description="视频模板绑定和模型参数只保存在当前 profile 下；新增、编辑、删除模型配置都会先进入草稿，点击保存后生效。"
                  dirty={videoDirty}
                  onSave={() => saveModule('视频配置')}
                  onReset={restoreVideoDefaults}
                />
                <VideoCapabilityManager
                  profile={activeProfile}
                  enabled={activeProviderIsOpenAICompatible}
                  onCommitProfilePatch={updateActiveProfile}
                />
              </div>
            )}
