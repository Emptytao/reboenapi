...
import { ALL_FAVORITES_COLLECTION_ID, deleteFavoriteCollection, getTaskFavoriteCollectionIds, useStore, submitTask, submitAgentMessage, stopAgentResponse, addImageFromFile, createInputImageFromFile, deleteImageIfUnreferenced, removeMultipleTasks, getCachedImage, ensureImageCached, getActiveAgentRounds, taskMatchesFilterStatus, taskMatchesSearchQuery, addVideoAssetFromFile, removeVideoDraftAsset, submitVideoGeneration } from '../store'
import { DEFAULT_PARAMS, type TaskRecord } from '../types'
import { getActiveApiProfile, getApiProfileForMode, normalizeSettings } from '../lib/apiProfiles'
import { DEFAULT_FAL_IMAGE_SIZE, getChangedParams, getOutputImageLimitForSettings, normalizeParamsForSettings } from '../lib/paramCompatibility'
import { getVideoModelCapability } from '../lib/videoCapabilities'
...
export default function InputBar() {
  ...
  const videoDraft = useStore((s) => s.videoDraft)
  const setVideoDraft = useStore((s) => s.setVideoDraft)
  const removeVideoDraftImage = useStore((s) => s.removeVideoDraftImage)
  const clearVideoDraftImages = useStore((s) => s.clearVideoDraftImages)
  const moveVideoDraftImage = useStore((s) => s.moveVideoDraftImage)
  ...
  const effectivePrompt = isVideoMode ? videoDraft.prompt : prompt
  const setEffectivePrompt = useCallback((value: string) => {
    if (isVideoMode) {
      setVideoDraft({ prompt: value })
    } else {
      setPrompt(value)
    }
  }, [isVideoMode, setPrompt, setVideoDraft])
  const effectiveInputImages = isVideoMode
    ? videoDraft.imageAssets.map((asset) => ({ id: asset.id, dataUrl: asset.objectUrl }))
    : inputImages
  ...
  const currentActiveProfile = useMemo(() => getApiProfileForMode(settings, appMode === 'agent' ? 'agent' : appMode === 'video' ? 'video' : 'gallery'), [settings, appMode])
  ...
  const videoModelOptions = useMemo(() => Array.from(new Set([
    activeProfile.model,
    videoDraft.modelId,
    ...(activeProfile.videoConfig?.availableModels ?? []),
    ...Object.keys(activeProfile.videoConfig?.capabilityOverrides ?? {}),
  ])).filter(Boolean), [activeProfile.model, activeProfile.videoConfig?.availableModels, activeProfile.videoConfig?.capabilityOverrides, videoDraft.modelId])
  const videoCapability = useMemo(() => isVideoMode ? getVideoModelCapability(activeProfile, videoDraft.modelId) : null, [activeProfile, isVideoMode, videoDraft.modelId])
  const hasSubmitApiConfig = Boolean(activeProfile.apiKey)
  const canSubmit = Boolean(effectivePrompt.trim() && hasSubmitApiConfig && !activeAgentIsRunning && (!isVideoMode || Boolean(videoCapability)))
  const submitDisabledReason = !hasSubmitApiConfig
    ? '尚未完成 API 配置，请在右上角设置中进行'
    : isVideoMode && !videoCapability
    ? '当前视频模型尚未绑定模板，请先到视频配置中保存模型配置'
    : ''
  const submitButtonAriaLabel = activeAgentIsRunning
    ...
    : '请先配置 API'
  const submitTooltipText = activeAgentIsRunning ? '停止生成' : submitDisabledReason
  const promptPlaceholder = isVideoMode ? '描述你想生成的视频，可输入 @ 来指定参考图...' : '描述你想生成的图片，可输入 @ 来指定参考图...'
  ...
  const syncPromptFromContentEditable = useCallback(() => {
    ...
    setEffectivePrompt(getContentEditablePlainText(el))
  }, [setEffectivePrompt])
  ...
  const isFalTextToImage = isFalProvider && !isVideoMode && inputImages.length === 0
  ...
  const videoImageLimit = videoCapability?.imageInput.max ?? 0
  const effectiveImageLimit = isVideoMode ? videoImageLimit : API_MAX_IMAGES
  const atImageLimit = effectiveInputImages.length >= effectiveImageLimit
  const uploadImageTooltipText = atImageLimit ? `参考图数量已达上限（${effectiveImageLimit} 张），无法继续添加` : '上传图片'
  ...
  const qualityHint = useHintTooltip({ enabled: () => currentActiveProfile.codexCli || isFalProvider })
  const nLimitHint = useHintTooltip({ autoHideMs: 2000 })
  const maskTargetImage = !isVideoMode && maskDraft
  ...
  const visiblePrompt = stripImageMentionMarkers(effectivePrompt)
  ...
  const atImageSourceCount = effectiveInputImages.length + agentOutputImageOptions.length
  const atImageQuery = isCursorInSelectedImageMention(effectivePrompt, cursorPosition)
    ? null
  ...
  const selectAtImageOption = useCallback((option: AtImageOption) => {
    const el = textareaRef.current
    const cursor = el ? getContentEditableCursor(el) : effectivePrompt.length
    const query = getAtImageQuery(stripImageMentionMarkers(effectivePrompt), cursor, { length: atImageSourceCount })
    ...
    const next = option.type === 'input'
      ? insertImageMentionAtVisibleRange(effectivePrompt, query.start, cursor, option.imageIndex)
      : insertTextMentionAtVisibleRange(effectivePrompt, query.start, cursor, option.insertText)
    isUserInputRef.current = false
    setEffectivePrompt(next.prompt)
    window.setTimeout(() => {
  ...
  }, [atImageSourceCount, effectivePrompt, setEffectivePrompt, syncPromptFromContentEditable])
  ...
  const insertPromptTextAtSelection = useCallback((text: string) => {
    ...
    const selection = el ? getContentEditableSelection(el) : { start: effectivePrompt.length, end: effectivePrompt.length }
    const promptStart = getPromptIndexFromVisibleIndex(effectivePrompt, selection.start)
    const promptEnd = getPromptIndexFromVisibleIndex(effectivePrompt, selection.end)
    const nextPrompt = `${effectivePrompt.slice(0, promptStart)}${text}${effectivePrompt.slice(promptEnd)}`
    const nextCursor = selection.start + text.length
    isUserInputRef.current = false
    setEffectivePrompt(nextPrompt)
    window.setTimeout(() => {
      if (textareaRef.current) {
        textareaRef.current.focus()
  ...
  }, [effectivePrompt, setEffectivePrompt, syncPromptFromContentEditable])
  ...
  const handleFiles = async (files: FileList | File[]) => {
    try {
      if (isVideoMode) {
        if (!videoCapability || videoCapability.imageInput.max <= 0) {
          showToast('当前视频模型不支持图片输入', 'error')
          return
        }
        const currentCount = useStore.getState().videoDraft.imageAssets.length
        if (currentCount >= videoCapability.imageInput.max) {
          showToast(`参考图数量已达上限（${videoCapability.imageInput.max} 张），无法继续添加`, 'error')
          return
        }
        const remaining = videoCapability.imageInput.max - currentCount
        const accepted = Array.from(files).filter((f) => f.type.startsWith('image/'))
        const toAdd = accepted.slice(0, remaining)
        const discarded = accepted.length - toAdd.length
        for (const file of toAdd) {
          await addVideoAssetFromFile(file, 'image')
        }
        if (discarded > 0) {
          showToast(`已达上限 ${videoCapability.imageInput.max} 张，${discarded} 张图片被丢弃`, 'error')
        }
        return
      }
  ...
  const handleVideoFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (!videoCapability || videoCapability.videoInput.max <= 0) {
        showToast('当前视频模型不支持视频素材输入', 'error')
        return
      }
      const currentCount = useStore.getState().videoDraft.videoAssets.length
      if (currentCount >= videoCapability.videoInput.max) {
        showToast(`视频素材数量已达上限（${videoCapability.videoInput.max} 个）`, 'error')
        return
      }
      const remaining = videoCapability.videoInput.max - currentCount
      const accepted = files.filter((file) => file.type.startsWith('video/'))
      const toAdd = accepted.slice(0, remaining)
      const discarded = accepted.length - toAdd.length
      for (const file of toAdd) {
        await addVideoAssetFromFile(file, 'video')
      }
      if (discarded > 0) showToast(`已达上限 ${videoCapability.videoInput.max} 个，${discarded} 个视频被丢弃`, 'error')
    } catch (err) {
      showToast(`视频素材添加失败：${err instanceof Error ? err.message : String(err)}`, 'error')
    }
  }
  ...
  const handlePromptCopy = (e: React.ClipboardEvent<HTMLDivElement>) => {
    ...
    const promptStart = getPromptIndexFromVisibleIndex(effectivePrompt, selection.start)
    const promptEnd = getPromptIndexFromVisibleIndex(effectivePrompt, selection.end)
    const text = stripImageMentionMarkers(effectivePrompt.slice(promptStart, promptEnd))
  ...
  useEffect(() => {
    ...
    const parts = getPromptMentionParts(effectivePrompt, effectiveInputImages)
    const html = effectivePrompt
      ? parts.map((part) =>
          part.type === 'mention'
  ...
  }, [effectivePrompt, effectiveInputImages])
  ...
  useEffect(() => {
    ...
    const needSentinel = effectivePrompt.endsWith('\n')
  ...
  }, [effectivePrompt, effectiveInputImages, adjustTextareaHeight, isMobile, mobileCollapsed])
  ...
  useEffect(() => {
    adjustTextareaHeight()
  }, [effectiveInputImages.length, Boolean(maskDraft), maskPreviewUrl, adjustTextareaHeight])
  ...
  const normalizeImageDropIndex = (idx: number) => {
    ...
    return Math.max(minIdx, Math.min(effectiveInputImages.length, idx))
  }
  ...
  const renderImageThumb = (img: (typeof effectiveInputImages)[number], idx: number) => {
    ...
    const isLast = idx === effectiveInputImages.length - 1
    const showDropBefore = imageDragOverIndex === idx && imageDragIndex !== idx
    const showDropAfter = imageDragOverIndex === effectiveInputImages.length && isLast && imageDragIndex !== idx
    ...
    const handleDrop = (e: React.DragEvent) => {
      ...
      if (fromIdx !== null && toIdx !== null) {
        if (isVideoMode) {
          moveVideoDraftImage(fromIdx, toIdx)
        } else {
    ...
    const handleTouchEnd = (e: React.TouchEvent) => {
      ...
      if (touchDrag.index !== null && imageDragOverIndexRef.current !== null) {
        e.preventDefault()
        if (isVideoMode) {
          moveVideoDraftImage(touchDrag.index, imageDragOverIndexRef.current)
        } else {
    ...
    return (
      <div
        ...
        onContextMenu={(e) => {
          ...
          const next = insertImageMentionAtVisibleRange(effectivePrompt, cursor, cursor, idx)
          isUserInputRef.current = false
          setEffectivePrompt(next.prompt)
          window.setTimeout(() => {
            if (textareaRef.current) {
      ...
      >
        ...
        <div
          ...
          onClick={() => {
            ...
            if (!isVideoMode) setLightboxImageId(img.id, inputImages.map((i) => i.id))
          }}
        >
          {displaySrc && (
          ...
          {canEdit && (
            <button 
              ...
              onClick={(e) => {
                e.stopPropagation()
                if (!isVideoMode) handleEditReferenceImage(img, idx, isMaskTarget)
              }}
      ...
      onClick={() =>
        setConfirmDialog({
          ...
          message: maskTargetImage
            ...
            : `确定要清空全部 ${effectiveInputImages.length} 张参考图吗？`,
          action: () => isVideoMode ? clearVideoDraftImages() : clearInputImages(),
        })
      }
    ...
    return (
      <div ref={imagesRef}>
        <div className="grid grid-cols-[repeat(auto-fill,52px)] justify-between gap-x-2 gap-y-3 mb-3">
          {effectiveInputImages.map((img, idx) => renderImageThumb(img, idx))}
  ...
  const renderVideoAssetChips = () => {
    if (!isVideoMode || videoDraft.videoAssets.length === 0) return null
    return (
      <div className="mb-3 flex flex-wrap gap-2">
        {videoDraft.videoAssets.map((asset, index) => (
          <div
            key={asset.id}
            className="group flex max-w-full items-center gap-2 rounded-xl border border-gray-200/60 bg-white/50 px-2.5 py-1.5 text-xs text-gray-600 shadow-sm dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-300"
            title={asset.name}
          >
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-gray-400 dark:bg-white/[0.06] dark:text-gray-500">
              <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
              </svg>
            </span>
            <span className="shrink-0 text-gray-400 dark:text-gray-500">视频{index + 1}</span>
            <span className="min-w-0 max-w-[14rem] truncate">{asset.name}</span>
            <button
              type="button"
              onClick={() => { void removeVideoDraftAsset('video', asset.id) }}
              className="ml-1 rounded-full p-0.5 text-gray-400 opacity-70 transition hover:bg-red-50 hover:text-red-500 group-hover:opacity-100 dark:hover:bg-red-950/30"
              aria-label={`移除视频素材 ${index + 1}`}
            >
  ...
  const renderVideoParams = (cols: string) => (
    <div className={`grid ${cols} gap-2 text-xs flex-1`}>
      <label className="flex min-w-0 flex-col gap-0.5">
        ...
        <Select
          value={videoDraft.modelId}
          onChange={(val) => setVideoDraft({ modelId: val })}
          options={videoModelOptions.length > 0
            ? videoModelOptions.map((model) => ({ label: model, value: model }))
            : [{ label: '未配置模型', value: '' }]}
      ...
      <label className="flex flex-col gap-0.5">
        ...
        <Select
          value={videoDraft.durationSeconds == null ? '' : String(videoDraft.durationSeconds)}
          onChange={(val) => setVideoDraft({ durationSeconds: val ? Number(val) : null })}
          options={(videoCapability?.durationOptions.length ? videoCapability.durationOptions : [videoDraft.durationSeconds].filter((value): value is number => value != null))
            .map((duration) => ({ label: `${duration}s`, value: String(duration) }))}
          disabled={!videoCapability}
          className={!videoCapability
      ...
      <label className="flex min-w-0 flex-col gap-0.5">
        ...
        <Select
          ...
          onChange={(val) => setVideoDraft({ resolution: val })}
          options={(videoCapability?.resolutionOptions.length ? videoCapability.resolutionOptions : [videoDraft.resolution].filter(Boolean))
            .map((resolution) => ({ label: resolution, value: resolution }))}
          disabled={!videoCapability}
          className={!videoCapability
            ? 'px-3 py-1.5 rounded-xl border border-gray-200/60 dark:border-white/[0.08] bg-gray-100/50 dark:bg-white/[0.05] opacity-50 cursor-not-allowed text-xs transition-all duration-200 shadow-sm'
            : selectClass}
  ...
  const renderParams = (cols: string) => isVideoMode ? renderVideoParams(cols) : (
    <div className={`grid ${cols} gap-2 text-xs flex-1`}>
      <label
      ...
      >
        ...
        <Select
          value={currentActiveProfile.codexCli ? 'auto' : isFalProvider && params.quality === 'auto' ? 'high' : params.quality}
          onChange={(val) => {
            if (!currentActiveProfile.codexCli) setParams({ quality: val as any })
          }}
          options={qualityOptions}
          disabled={currentActiveProfile.codexCli}
          className={currentActiveProfile.codexCli
            ? 'px-3 py-1.5 rounded-xl border border-gray-200/60 dark:border-white/[0.08] bg-gray-100/50 dark:bg-white/[0.05] opacity-50 cursor-not-allowed text-xs transition-all duration-200 shadow-sm'
            : selectClass}
        />
        <ButtonTooltip
          visible={(currentActiveProfile.codexCli || isFalProvider) && qualityHint.visible}
  ...
  const showFavoriteCollectionBatchBar = !isVideoMode && inCollectionOverview && selectedFavoriteCollectionIds.length > 0
  const showTaskBatchBar = !isVideoMode && !showFavoriteCollectionBatchBar && selectedTaskIds.length > 0
  const videoFileLimit = videoCapability?.videoInput.max ?? 0
  const atVideoLimit = !isVideoMode || videoFileLimit <= 0 || videoDraft.videoAssets.length >= videoFileLimit
  const atUploadLimit = isVideoMode ? atImageLimit && atVideoLimit : atImageLimit
  const uploadVideoTooltipText = videoFileLimit <= 0
    ? '当前视频模型不支持视频素材'
    : atVideoLimit
    ? `视频素材数量已达上限（${videoFileLimit} 个）`
    : '上传视频素材'
  const submitButtonText = activeAgentIsRunning ? '停止生成' : isVideoMode ? '生成视频' : maskDraft ? '遮罩编辑' : '生成图像'
        ...
        <div className="fixed inset-0 z-[100] bg-white/60 dark:bg-gray-900/60 backdrop-blur-md flex flex-col items-center justify-center pointer-events-none">
          <div className="flex flex-col items-center gap-4 p-8 rounded-3xl">
            ...
            <div className="text-center">
              {atImageLimit ? (
                <>
                  <p className="text-lg font-semibold text-red-500">已达上限 {effectiveImageLimit} 张</p>
              ...
              <>
                ...
                {mobileCollapsed && (
                  <div className="text-xs text-gray-400 dark:text-gray-500 mb-2 ml-1">
                    {maskTargetImage ? `1 张遮罩主图 · ${referenceImages.length} 张参考图` : `${effectiveInputImages.length} 张参考图`}
                  </div>
                )}
              </>
            ) : (
          ...
          <div className="mt-3">
            ...
            <div className="hidden sm:flex items-end justify-between gap-3">
              ...
              <div className="flex gap-2 flex-shrink-0 mb-0.5">
                ...
                {isVideoMode && videoFileLimit > 0 && (
                  <div
                    className="relative"
                    onMouseEnter={() => setAttachHover(true)}
                    onMouseLeave={() => setAttachHover(false)}
                  >
                    <ButtonTooltip visible={attachHover} text={uploadVideoTooltipText} />
                    <button
                      onClick={() => !atVideoLimit && videoFileInputRef.current?.click()}
                      className={`p-2.5 rounded-xl transition-all shadow-sm ${
                        atVideoLimit
                          ? 'bg-gray-200 dark:bg-white/[0.04] text-gray-300 dark:text-gray-500 cursor-not-allowed'
                          : 'bg-gray-200 dark:bg-white/[0.06] hover:bg-gray-300 dark:hover:bg-white/[0.1] text-gray-500 dark:text-gray-300 hover:shadow'
                      }`}
                      aria-label={uploadVideoTooltipText}
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                      </svg>
                ...
                >
                  <ButtonTooltip visible={(activeAgentIsRunning || Boolean(submitDisabledReason)) && submitHover} text={submitTooltipText} />
                  <button
                    onClick={() => activeAgentIsRunning ? stopActiveAgentResponse() : hasSubmitApiConfig ? submitCurrentMode() : setShowSettings(true, 'api')}
                  ...
                  {showMobileUploadMenu && (
                    <>
                      ...
                      <div className="absolute bottom-full left-0 mb-2 w-32 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
                        ...
                        {isVideoMode && videoFileLimit > 0 && (
                          <button
                            className="w-full px-4 py-3 text-left text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50 disabled:cursor-not-allowed disabled:opacity-45 flex items-center gap-2 transition-colors"
                            onClick={() => {
                              setShowMobileUploadMenu(false)
                              if (!atVideoLimit) videoFileInputRef.current?.click()
                            }}
